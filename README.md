...
| `ghcr.io/valqore/engine-ai:1.13.0` | **Licensed** ([request access](mailto:tunc@valqore.io)) | Everything above + embedded offline AI model (AI scan, chat) |
...
docker run --rm -v valqore-data:/app/data ghcr.io/valqore/engine-ai:1.13.0 valqore activate YOUR_LICENSE_KEY
```
...
**Public image** (`ghcr.io/valqore/engine:1.13.0`) — keyless-signed in CI via Sigstore (GitHub OIDC + Rekor):

```bash
cosign verify ghcr.io/valqore/engine:1.13.0 \
  --certificate-identity-regexp 'https://github.com/valqore/valqore-engine/.*' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
cosign verify-attestation ghcr.io/valqore/engine:1.13.0 --type spdxjson \
  --certificate-identity-regexp 'https://github.com/valqore/valqore-engine/.*' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
```

**AI image** (`ghcr.io/valqore/engine-ai:1.13.0`) — signed with Valqore's release key ([`cosign.pub`](cosign.pub)):

```bash
cosign verify --key cosign.pub --insecure-ignore-tlog ghcr.io/valqore/engine-ai:1.13.0
```
...
The `ghcr.io/valqore/engine-ai:1.13.0` image bundles a fine-tuned model that runs **fully offline** — your code never leaves the container. [Request a license.](mailto:tunc@valqore.io)

**AI scan — evaluate + drift + plain-English explanation in one shot:**

```bash
docker run --rm -v "$PWD:/work" -w /work -v valqore-data:/app/data \
  ghcr.io/valqore/engine-ai:1.13.0 valqore ai-scan ./ --state terraform.tfstate --cloud aws
```
...
docker run --rm -it -v "$PWD:/work" -w /work -v valqore-data:/app/data \
  ghcr.io/valqore/engine-ai:1.13.0 valqore chat deploy.yaml
```
...
docker run --rm -v "$PWD:/work" -w /work -v valqore-data:/app/data \
  ghcr.io/valqore/engine-ai:1.13.0 valqore ai-scan examples/ai-scan/
```
...
docker run --rm -it -v "$PWD:/work" -w /work -v valqore-data:/app/data \
  ghcr.io/valqore/engine-ai:1.13.0 valqore chat examples/chat/healthcare-api.yaml
