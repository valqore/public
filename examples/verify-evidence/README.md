...
Valqore produced when it evaluated that pod (`evidence.json`): the frozen input, all 517
rule outcomes, and the control statuses for every one of the 19 built-in compliance
frameworks (169 control statuses), sealed under one SHA-256 content hash.
...
```bash
# requires Valqore Engine 1.19.0 (the version that produced this bundle), e.g.:
docker run --rm -v "$PWD:/w" ghcr.io/valqore/engine:1.19.0 valqore verify /w/evidence.json
# -> REPRODUCED -- 517 outcomes re-ran identically; verdict BLOCK. 169 control statuses reproduced.
```
...
docker run --rm -v "$PWD:/w" ghcr.io/valqore/engine:1.19.0 valqore verify /w/t1.json   # MISMATCH, exit 2
...
docker run --rm -v "$PWD:/w" ghcr.io/valqore/engine:1.19.0 valqore verify /w/t2.json   # hash broken AND replay disagrees
...
docker run --rm -v "$PWD:/w" ghcr.io/valqore/engine:1.19.0 valqore verify /w/t3.json   # control MISMATCH, exit 2
```
