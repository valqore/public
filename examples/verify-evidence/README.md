...
```bash
# requires Valqore Engine 1.15.0 (the version that produced this bundle), e.g.:
docker run --rm -v "$PWD:/w" ghcr.io/valqore/engine:1.15.0 valqore verify /w/evidence.json
...
docker run --rm -v "$PWD:/w" ghcr.io/valqore/engine:1.15.0 valqore verify /w/t1.json   # MISMATCH, exit 2
...
docker run --rm -v "$PWD:/w" ghcr.io/valqore/engine:1.15.0 valqore verify /w/t2.json   # hash broken AND replay disagrees
...
docker run --rm -v "$PWD:/w" ghcr.io/valqore/engine:1.15.0 valqore verify /w/t3.json   # control MISMATCH, exit 2
```
